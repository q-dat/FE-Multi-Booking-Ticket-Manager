...
const TicketFlightsResultsPage: React.FC = () => {
    ...
    const fetchTickets = async () => {
      try {
        ...
        if (storedTickets) {
          ...
          const initialFlightName = updatedTickets[0]?.seat_id[0]?.seat_catalog_id.vehicle_id.name || null;
  ...
  const handleFlightChange = (flightName: string) => { // Thêm hàm xử lý thay đổi chuyến bay
    ...
    setSelectedClassId(ticketsByFlight[flightName][0]?.seat_id[0]?.seat_catalog_id._id || null);
  };
