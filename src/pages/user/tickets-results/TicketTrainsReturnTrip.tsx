...
const TicketTrainsReturnTrip: React.FC = () => {
  const { searchTickets } = useContext(TicketContext);
  const { t } = useTranslation();
  const [alternateTickets, setAlternateTickets] = useState<ITicket[]>([]);
  ...
  useEffect(() => {
    const storedTickets = localStorage.getItem('searchResultsAlternate');
    if (storedTickets) {
      const parsedTickets = JSON.parse(storedTickets) as ITicket[];
      setAlternateTickets(parsedTickets);
    } else {
      setAlternateTickets([]);
    }
    //
  ...
  // if (alternateTickets.length === 0) {
  //   return <LoadingLocal />;
  // }
  console.log(alternateTickets);

  //
  const tripInfo = alternateTickets[0]?.trip_id;
  //Ticket By Vehicle
  const ticketsByTrain = alternateTickets.reduce(
    (acc, ticket) => {
  ...
  const ticketsByCarriage =
    selectedTrain && ticketsByTrain[selectedTrain]
      ...
      <div className="w-full">
        ...
        <div className="mt-5 w-full">
          <h1 className="mx-2 mb-5 border-[4px] border-b-0 border-r-0 border-t-0 border-primary bg-blue-200 px-5 py-1 text-center text-xl text-black dark:border-white dark:bg-gray-400 dark:text-white xl:text-start">
            {t('UserPage.TicketTrainsResults.TripReturn')}
...
export default TicketTrainsReturnTrip;
