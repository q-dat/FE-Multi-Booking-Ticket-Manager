...
export const getAllPaymentsApi = async (): Promise<IPayment[]> => {
  try {
    const response = await axios.get<{ payments: IPayment[] }>(
      '/api/payments/all'
    );
...
export const getPaymentByOrderIdApi = async (
  orderId: string
): Promise<IPayment> => {
  if (!orderId) throw new Error('Order ID is required');
  try {
    const response = await axios.get<{ payment: IPayment }>(
...
export const createPaymentApi = async (
  ...
  try {
    const response = await axios.post<{ payment: IPayment }>(
      '/api/payments',
      payment
    );
    return response.data.payment;
...
export const updatePaymentStatusApi = async (
  ...
  try {
    const response = await axios.put<{ payment: IPayment }>(
